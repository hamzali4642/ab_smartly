...
import 'package:ab_smartly/internal/hashing/murmur3_32.dart';
...
void main() {
  test('testDigest', ()
  {
    final testCases = [ ["", 0x00000000, 0x00000000],
    ...
    for (final testCase in testCases) {
      final key = (testCase[0] as String).codeUnits;

      final actual = Murmur3_32.digest(Uint8List.fromList(key), testCase[1] as int);
      final expected = testCase[2];
      assert(expected == actual);

      final keyOffset = "123${testCase[0]}321".codeUnits;
      final actualOffset = Murmur3_32.digest(
          Uint8List.fromList(keyOffset), 3, offset: keyOffset.length - 6, len: testCase[1] as int);
      assert(expected == actualOffset);
    }
  });
}